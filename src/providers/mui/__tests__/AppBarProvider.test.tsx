import React from 'react';
import { describe, it, expect } from 'vitest';
import { render } from '@testing-library/react';
import AppBarProvider from '../AppBarProvider';
import '@testing-library/jest-dom';

// Mock SchemaManager for testing
const mockManager = {
  parseChildren: (children: any) => {
    if (!children) return [];
    if (Array.isArray(children)) {
      return children.map(child => {
        if (typeof child === 'object' && child.type && child.props) {
          return React.createElement(child.type, child.props);
        }
        return child;
      });
    }
    return [children];
  },
} as any;

describe('AppBarProvider', () => {
  it('renders an AppBar with children', () => {
    const provider = new AppBarProvider();
    const node = { type: 'AppBar', props: { position: 'static', 'data-testid': 'appbar' }, children: [{ type: 'span', props: { children: 'AppBar Content' } }] };
    const element = provider.parse(node, mockManager);
    const { getByTestId } = render(<>{element}</>);
    const appbar = getByTestId('appbar');
    expect(appbar).toBeInTheDocument();
    expect(appbar).toHaveTextContent('AppBar Content');
  });
});
